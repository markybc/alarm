...
func GetAlarmNoticeListenerListByAccessToken(accessToken string, status interface{}, o orm.Ormer) []AlarmNoticeMsg {
	...
	qs := o.QueryTable("alarm_notice_msg")
	if status != nil {
		qs.Filter("status", status).Filter("access_token", accessToken).Filter("create_time__gt", d1).OrderBy("id").Limit(50).All(&msgList)
	} else {
		qs.Filter("access_token", accessToken).Filter("create_time__gt", d1).OrderBy("id").Limit(50).All(&msgList)
